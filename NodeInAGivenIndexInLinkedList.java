...
class GfG
{
    ...
    {
       int count = 1; // Start count from 1 (1-based index)
        
        // Traverse the list until you reach the desired index
        while (node != null) {
            if (count == ind) {
                return node.data; // Return the data at the given index
            }
            count++;
            node = node.next; // Move to the next node
        }
        
        // If the index is out of range, return -1 or throw an exception as needed
        return -1;
    }
}